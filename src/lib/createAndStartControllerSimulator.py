...
experiment_config.name = "dubins_car"
experiment_config.file_name = proj.project_root+'configs/dubins_car.config'
...
try:
    ...
    robot_config.fromData({'RobotName':['dubins'],
                          'Type':['unicycleSim'],
                          'CalibrationMatrix':'[[{scale}, 0, {offset_x}],\
                                                 [0, {scale}, {offset_y}],\
                                                 [0, 0, 1]]'.format(scale=scale, offset_x=offset[0],offset_y=offset[1]),
                          'InitHandler': ['unicycleSim.BasicSimInitHandler(init_region="{start_region}",\
                                         x=0.0, y=0.0, theta=0.0)'.format(start_region=start_list[0])]}, hsub)

except handlerTemplates.LoadingError, msg:
    ltlmop_logger.error(msg)
robot_config.fromFile(LTLMoP_dir+"src/lib/handlers/unicycleSim/basicSim.robot", hsub)

experiment_config.robots = [robot_config]
experiment_config.main_robot = "basicSim"
...
# hsub.handler_configs['eagent'] = {handlerTemplates.SensorHandler:[]}
# hsub.handler_configs['eagent'][handlerTemplates.SensorHandler].append(
#     hsub.loadHandler('eagent', handlerTemplates.SensorHandler, 'DummySensorHandler'))
...
if agent_prop_list:
    ...
    m = copy.deepcopy(hsub.handler_configs["share"][handlerTemplates.SensorHandler][0].getMethodByName("buttonPress"))
    para = m.getParaByName("button_name")
    para.setValue('agentAtNextIntersection')
    para = m.getParaByName("init_value")
    para.setValue(False)    
    prop_mapping["agentAtNextIntersection"] = hsub.method2String(m, "share")

    # agentInSameLaneInFrontOfMe
    m = copy.deepcopy(hsub.handler_configs["share"][handlerTemplates.SensorHandler][0].getMethodByName("buttonPress"))
    para = m.getParaByName("button_name")
    para.setValue('agentInSameLaneInFrontOfMe')
    para = m.getParaByName("init_value")
    para.setValue(False)    
    prop_mapping["agentInSameLaneInFrontOfMe"] = hsub.method2String(m, "share")
